    ...
    },
    {
      ...
      getter: function() {
        ...
        if (desc.get !== undefined) {
          // Some custom getter/setters throw when invoked on prototypes.
          try {
            return desc.get();
          } catch (err) {
            return undefined;
      ...
      if (this.set) {
        if (this.get) {
          newDesc.set = function(newValue) {
            return (hook.value = hook.set(hook.get(), newValue));
          };
        } else {
          newDesc.set = function(newValue) {
            return (hook.value = hook.set(hook.value, newValue));
          };
        }
      } else {
        newDesc.set = function(newValue) {
          return (hook.value = newValue);
        };
      }
